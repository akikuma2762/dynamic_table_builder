...
// 使用 Quill composable
const { quillInstance, isClient } = useQuill(quillEditor, quillToolbar)
...
// 監聽 Quill 實例變化，設置事件監聽器
watch(quillInstance, (newQuill) => {
  if (newQuill) {
    newQuill.on('text-change', syncQuillToPreview)
    syncQuillToPreview()
    loadCustomPalette()
  }
})
...
function syncQuillToPreview() {
  if (!quillInstance.value || !builderPreview.value) return
  const html = quillInstance.value.root.innerHTML.trim()
...
function addCustomCheckbox() {
  ...
  const index = quillInstance.value.getSelection()?.index ?? quillInstance.value.getLength()
  quillInstance.value.insertEmbed(index, 'checkbox', { label })
...
function addCustomInput() {
  ...
  const index = quillInstance.value.getSelection()?.index ?? quillInstance.value.getLength()
  quillInstance.value.insertEmbed(index, 'textinput', { placeholder: ph, width: '100px' })
  quillInstance.value.insertText(index + 1, ' ')
  syncQuillToPreview()
}
...
function saveCustomPalette() {
  if (!isClientSide()) return
  safeSetLocalStorage('paletteCustom', customItems.value)
}
</script>
